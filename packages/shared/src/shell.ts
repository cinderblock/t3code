...
// Memoize resolved spawn-executable paths. Resolution walks PATH with synchronous statSync
// on every spawn (Windows), so repeatedly spawning the same command (e.g. `git` during status
// polling) otherwise re-scans PATH every time and blocks the event loop. Keyed by
// platform + command + PATH; only successful resolutions are cached so a not-yet-installed
// command stays re-checkable. Bounded and cleared wholesale on overflow (entries are stable).
const spawnExecutableResolutionCache = new Map<string, string>();
const SPAWN_EXECUTABLE_RESOLUTION_CACHE_MAX = 256;
...
function resolveSpawnExecutableWithNode(
  ...
  if (resolved !== undefined) {
    if (spawnExecutableResolutionCache.size >= SPAWN_EXECUTABLE_RESOLUTION_CACHE_MAX) {
      spawnExecutableResolutionCache.clear();
    }
    spawnExecutableResolutionCache.set(cacheKey, resolved);
  }
  return resolved;
}

function resolveSpawnExecutableWithNodeUncached(
  command: string,
  platform: NodeJS.Platform,
  env: NodeJS.ProcessEnv,
): string | undefined {
