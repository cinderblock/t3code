...
**Trap:** the first version *cancelled* the initial refresh instead of delaying it; when
...
- **`Effect.async` does not exist in Effect 4** — it is `Effect.callback`. Using the wrong name
  ...
  and surfaces as ~60 errors *in another file*. Bisect by disabling the new layer.
