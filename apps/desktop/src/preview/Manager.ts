    ...
    const install = Effect.fn("PreviewManager.installWebContentsListeners")(function* () {
      yield* attempt({ operation: "attachListeners", tabId, webContentsId: wc.id }, () => {
        ...
        wc.setWindowOpenHandler((details) => {
          // A scripted popup (OAuth-style) must stay in-app: the flow completes
          // through its opener, which the system browser cannot reach. The
          // external-link policy below only governs ordinary link-style opens.
          ...
          const target = externalTargetFor(details.url);
