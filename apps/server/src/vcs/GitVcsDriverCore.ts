...
// Upstream implemented the same fetch-failure backoff the fork carried, with its
// own helpers and a more conservative window (30s/15min vs the fork's 5s/5min).
// Took upstream's wholesale; the fork's version is superseded.
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const fetchRemoteForStatus = (
    ...
    // Upstream's call shape (a non-zero exit is now a failure so the backoff can
    // see it, rather than allowNonZeroExit), wrapped in the fork's semaphore.
    // Upstream backs off REPEAT fetches per remote; the semaphore caps how many
    // DISTINCT repos fetch at once, which is what stops N repos from firing N
    // simultaneous SSH handshakes at startup.
