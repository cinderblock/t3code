...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  ...
  it.effect("retries Git root discovery after a failed lookup", () => {
    ...
    return Effect.gen(function* () {
      ...
      // A failed root lookup is cached on the negative TTL rather than retried on the very
      // next resolve — under load the lookup fails by timing out, and immediate retries feed
      // the load that caused the timeout. See plans/process-spawn-storm.md.
      expect(yield* resolver.resolve("/repo/packages/web")).toBeNull();
