...
export default function ChatView(props: ChatViewProps) {
  ...
  const serverQueuedMessages = useAtomValue(primaryQueuedMessagesAtom);
  ...
  useEffect(() => {
    ...
    const payload =
      rateLimitActivity.payload && typeof rateLimitActivity.payload === "object"
        ...
        : null;
    // `rejected` is what the server derives from the provider's usage windows;
    // `rateLimitInfo.status` is the shape activities persisted before that
    // normalization carry, kept so an old thread still converts.
    const rateLimitInfo =
      payload?.rateLimitInfo && typeof payload.rateLimitInfo === "object"
        ? (payload.rateLimitInfo as Record<string, unknown>)
        : null;
    if (payload?.rejected !== true && rateLimitInfo?.status !== "rejected") return;
    ...
    const limitWindows = (
      payload?.limits && typeof payload.limits === "object"
        ? ((payload.limits as { windows?: unknown }).windows ?? [])
        : []
    ) as ReadonlyArray<{ readonly kind?: unknown; readonly usedPercent?: unknown }>;
    const rejectedWindowKind =
      limitWindows.find(
  ...
  const rightPanelContent = activeThreadRef ? (
    ...
    ) : renderedRightPanelSurface?.kind === "gitGraph" ? (
      <Suspense fallback={null}>
        <GitGraphPanel
          environmentId={environmentId}
          cwd={gitCwd}
